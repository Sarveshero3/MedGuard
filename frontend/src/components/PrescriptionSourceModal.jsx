...
export function PrescriptionSourceModal({ medicine, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-900/60 backdrop-blur-xs animate-fade-in text-left overscroll-contain overflow-hidden">
      <div className="bg-white border border-slate-200 rounded-2xl shadow-2xl max-w-2xl w-full overflow-hidden flex flex-col max-h-[85vh] sm:max-h-[90vh]">
        {/* Modal Header */}
        ...
        <div className="p-4 sm:p-6 overflow-y-auto space-y-4 sm:space-y-6 flex-grow overscroll-contain">
          ...
          <div className="bg-slate-50/80 border border-slate-200/80 rounded-xl p-3 sm:p-4 grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 text-xs">
            <div>
          ...
          <div className="border border-slate-200 rounded-xl overflow-hidden bg-slate-100/70 min-h-[200px] sm:min-h-[260px] flex flex-col items-center justify-center p-2 sm:p-4">
            {isRealFile ? (
              isPdf ? (
                <object
                  data={documentUrl}
        ...
        {/* Modal Footer */}
        <div className="bg-slate-50 border-t border-slate-200 px-4 sm:px-6 py-3 sm:py-3.5 flex items-center justify-between flex-shrink-0">
          {isRealFile ? (
            <button
              type="button"
              onClick={downloadDocument}
              className="bg-[#0f766e] hover:bg-[#0d645c] text-white font-semibold text-xs px-3.5 sm:px-4 py-2 rounded-xl flex items-center gap-1.5 cursor-pointer shadow-sm transition-all hover:scale-[1.02] active:scale-[0.98]"
            >
            ...
            <button
              type="button"
              disabled
              className="bg-slate-200 text-slate-400 font-semibold text-xs px-3.5 sm:px-4 py-2 rounded-xl flex items-center gap-1.5 cursor-not-allowed"
            ...
            >
              ...
              <span className="hidden sm:inline">No File Attached</span>
              <span className="sm:hidden">No File</span>
            </button>
          )}
          <button
            type="button"
